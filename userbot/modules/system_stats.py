...
import psutil
from userbot import ALIVE_LOGO, ALIVE_NAME, BOT_VER, CMD_HELP, STEADY_TEKS_KUSTOM, StartTime, UPSTREAM_REPO_BRANCH, bot
...
@register(outgoing=True, pattern=r"^\.(?:steadyalive)\s?(.)?")
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"\n__**{STEADY_TEKS_KUSTOM}**__\n"
        f"**╭───────────────────**\n"
        f"**├[• 🤴 Steady Owner** \n"
        f"├[•   : `{DEFAULTUSER}` \n"
        f"**├[• 🔎 Username** \n"
...
async def amireallyalive(alive):
    ...
    output = (
        ...
        f"╔══════════╬════╬══════════╗ \n"
        f"╟⟩⟩ 🤴 • `ᴏᴡɴᴇʀ    :`[Steady](t.me/vckyclonee)             ㅤ ║\n"
        ...
        f"╚══════════╬════╬══════════╝ \n"
        f"🔰 : [Repo-Steady](https://github.com/5GVckyy/SteadyUserbot)\n👥 : [Support](t.me/steadysupportgroup)\n👤 : [Owner](t.me/Vckyclonee)\n"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
CMD_HELP.update({
    "alive":
    "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.alive` or `.on` or `.steadyalive`"
